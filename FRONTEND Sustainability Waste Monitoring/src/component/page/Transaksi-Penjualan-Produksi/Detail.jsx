...
export default function TransaksiPenjualanProduksiDetail({
  onChangePage,
  withID,
}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Respon API detailProdukPenjualan:", data);

        if (data === "ERROR") {
          throw new Error("Terjadi kesalahan: Gagal mengambil produk.");
        } else {
          setProduk(data);
        }
      } catch (error) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const data = await UseFetch(
          API_LINK + "TransaksiPenjualanProduk/DetailPenjualanProduk",
          { id: withID }
        );
  ...
  const mappedProduk = produk.map((item, index) => ({
    No: index + 1,
                ...
                <div className="border rounded-0 shadow-sm p-0 bg-white">
                  <TableInput
                    ...
                    renderCell={(rowIndex, key) => {
                      const row = mappedProduk[rowIndex];
                      return (
