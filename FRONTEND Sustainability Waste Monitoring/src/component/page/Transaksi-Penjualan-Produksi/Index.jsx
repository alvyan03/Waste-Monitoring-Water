...
const dataFilterSort = [
  ...
  { Value: "[Nama Produk] desc", Text: "Nama Profuk [↓]" },
];
...
export default function TransaksiPenjualanProduksiIndex({ onChangePage }) {
    ...
    const fetchDokumen = async () => {
      try {
        const response = await UseFetch(
          API_LINK + "TransaksiPenjualanProduk/cekDokumenPenjualanProduk",
          {}
        );
        setDokumen(response);
      } catch (error) {
    ...
    const fetchData = async () => {
      ...
      try {
        const data = await UseFetch(
          API_LINK + "TransaksiPenjualanProduk/GetDataPenjualanProduk",
          currentFilter
        );
  ...
  async function handleSent(id) {
    ...
    const pesan = dokumenLengkap
      ? "Apakah Anda yakin ingin tetap mengirim Data Penjualan Produk ini?"
      : "Anda belum melengkapi data pada bagian Dokumen.\nApakah Anda yakin ingin tetap mengirim Data Penjualan Produk ini?";

    const result = await SweetAlert(
      "Kirim Data Penjualan Produk",
      pesan,
      "warning",
      "Ya, Saya Yakin!"
    );
    ...
    if (result) {
      setIsLoading(true);
      UseFetch(API_LINK + "TransaksiPenjualanProduk/sentPenjualanProduk", {
        idPembuangan: id,
      })
        .then((data) => {
          if (data === "ERROR") setIsError(true);
            ...
            SweetAlert(
              "Sukses",
              "Data Penjualan Produksi berhasil dikirim",
              "success"
            );
      ...
      <div className="d-flex flex-column">
        {isError && (
          <div className="flex-fill">
            <Alert
              type="warning"
              message="Terjadi kesalahan: Gagal mengambil data penjualan produk."
            />
          </div>
        )}
        <div className="flex-fill">
